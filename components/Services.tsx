...
const ServiceListCategory: React.FC<{ title: string; items: string[] }> = ({ title, items }) => (
  <div className="mb-8">
    <h4 className="text-bear-accent font-black uppercase tracking-wider mb-4 border-b border-gray-800 pb-2 text-sm">{title}</h4>
    <ul className="space-y-2">
      {items.map((item, index) => (
        <li key={index} className="flex items-start gap-2 text-gray-400 text-xs sm:text-sm hover:text-white transition-colors">
          <CheckCircle2 size={14} className="text-bear-700 mt-0.5 shrink-0" />
          <span>{item}</span>
        </li>
      ))}
    </ul>
      ...
      <div className="bg-bear-900 py-20 border-t border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
                <h3 className="text-2xl font-black text-white uppercase tracking-tight">Comprehensive Service Capabilities</h3>
                <p className="text-gray-500 mt-2">Serving Denver, Englewood, and Littleton with specialized mechanical expertise.</p>
            </div>
